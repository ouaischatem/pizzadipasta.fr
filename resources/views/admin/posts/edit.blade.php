...
<!-- Permet la mise à jour d'un poste déjà existant -->
@section('content')
    <h1 class="text-3xl font-medium text-gray-700">Modification poste</h1>
    <div class="container mx-auto mt-10 bg-white p-3">
        ...
        <form method="post" action="{{ route('admin.posts.edit', $post->id) }}" enctype="multipart/form-data">
            ...
            <div class="mb-4">
                ...
                <img class="pt-2 max-h-64 mb-2" src="{{ url('storage/post_images/' . basename($post->image)) }}"
                     alt="Post Image">
            </div>
            ...
            <div class="mb-4">
                <label class="block mb-2 text-sm font-medium font-sans text-gray-600" for="content">Contenu du
                    poste</label>
                <textarea id="htmeditor" name="content" required>{{ $post->content }}</textarea>
            </div>
            ...
            <div class="mb-4">
                ...
                <textarea id="quote"
                          ...
                          name="quote"
                          placeholder="Citation" type="text"> {{ $post->quote }}</textarea>
            </div>

            <div class="mb-4">
                <label class="flex items-center">
                    <input type="checkbox" name="visible" class="form-checkbox" {{ $post->visible ? 'checked' : '' }}>
